...
namespace DoubleEngine
{
    ...
    sealed public partial record MeshFragmentVec2D : IMeshFragment<Vec2D>
    {
        [JsonIgnore] internal static readonly Vec2D[] EmptyVertices = new Vec2D[0];
        [JsonIgnore] internal static readonly int[] EmptyTriangles = new int[0];

        [JsonIgnore] public static readonly MeshFragmentVec2D Empty = new MeshFragmentVec2D();

        private MeshFragmentVec2D()
        {
        ...
        public bool Equals(MeshFragmentVec2D other)
        {
        ...
        //    return true;
	    return EqualityComparer<Vec2D[]>.Default.Equals(vertices, other.vertices) && 
            EqualityComparer<int[]>.Default.Equals(triangles, other.triangles);
        }

        public override int GetHashCode()
        {
	        return ((EqualityComparer<Type>.Default.GetHashCode(EqualityContract) * -1521134295 + EqualityComparer<Vec2D[]>.Default.GetHashCode(vertices)) * -1521134295 + EqualityComparer<int[]>.Default.GetHashCode(triangles));
        }



        [JsonProperty(Required = Required.Always)] internal readonly Vec2D[] vertices;
        //[Obsolete("if you need to fiddle with vertices then probably is something wrong")]
        [JsonIgnore] public ReadOnlySpan<Vec2D> Vertices { get {return new ReadOnlySpan<Vec2D>(vertices);} }
        ...
        private MeshFragmentVec2D(Vec2D[] vertices, int[] triangles)
        {
        ...
        [JsonConstructor]
        internal MeshFragmentVec2D(Vec2D[] vertices, int[] triangles, double joinedVertices): this(vertices, triangles)
        {
            if(vertices is null)
